    ...
    layui.config({
        ...
        $.post("{{url('/api/user/store')}}",
        {
            ...
            if(res.status==1){
                ...
                $('.layui-row.box3 .layui-form-item').eq(4).find('.layui-form-mid').html(res.data.account_info.sub_bank_name);
                $('.layui-row.box3 .layui-form-item').eq(5).find('.layui-form-mid').html(res.data.account_info.bank_no);//联行号
                $('.layui-row.box3 .layui-form-item').eq(6).find('.layui-form-mid').html(res.data.account_info.store_alipay_account);//支付宝账户
                // 四部分
