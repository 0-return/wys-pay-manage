        ...
        var uploadInst = upload.render({
            ...
            before : function(input){
                ...
                if(res.status == 0){
                    ...
                    layui.jquery('.item14').attr("src", res.data.store_license_no);
                    layui.jquery('.item22').attr("src", res.data.store_license_stime);
                    layui.jquery('.item15').attr("src", res.data.store_license_time);
                }
                //console.log(res); //上传成功返回值，必须为json格式
            }
        });
