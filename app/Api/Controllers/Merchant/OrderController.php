...
class OrderController extends BaseController
{
    ...
    public function order_data(Request $request)
    {
        try {
            ...
            if ($merchant->merchant_type == 2) {
                $old_month_order_data = MerchantStoreMonthOrder::where('month', $fm_forward_month)
                    ->where('merchant_id', $merchant_id)
                    ->where('store_id', $store_id)
                    ->select('total_amount', 'order_sum')
                    ->get();
                ...
                $old_month_order_data = StoreMonthOrder::where('month', $fm_forward_month)
                    ->whereIn('store_id', $store_ids)
                    ->select('total_amount', 'order_sum')
                    ->get();
            ...
            $old_month_count = '' . $old_month_order_data->sum('order_sum') . '';
            ...
            if ($merchant->merchant_type == 2) {
                $month_order_data = MerchantStoreMonthOrder::where('month', $tmonth)
                    ->where('merchant_id', $merchant_id)
                    ->where('store_id', $store_id)
                    ->select('total_amount', 'order_sum');
                ...
                $month_order_data = StoreMonthOrder::where('month', $tmonth)
                    ->whereIn('store_id', $store_ids)
                    ->select('total_amount', 'order_sum');
            ...
            $month_count = '' . $month_order_data->sum('order_sum') . '';
